...
suraj.hexgl.ShipControls = function(ctx)
{
	var self = this;
	...
	if(ctx.controlType == 1 && suraj.controllers.TouchController.isCompatible())
	{
		this.touchController = new suraj.controllers.TouchController(
	...
	else if(ctx.controlType == 4 && suraj.controllers.OrientationController.isCompatible())
	{
		this.orientationController = new suraj.controllers.OrientationController(
			domElement, true,
			function(state, touch, event){
				if(event.touches.length >= 4)
					window.location.reload(false);
	...
	else if(ctx.controlType == 3 && suraj.controllers.GamepadController.isCompatible())
	{
		this.gamepadController = new suraj.controllers.GamepadController(
      function(controller){
...
suraj.hexgl.ShipControls.prototype.control = function(threeMesh)
{
	this.mesh = threeMesh;
...
suraj.hexgl.ShipControls.prototype.reset = function(position, rotation)
{
...
suraj.hexgl.ShipControls.prototype.terminate = function()
{
	this.destroy();
...
suraj.hexgl.ShipControls.prototype.destroy = function()
{
	suraj.Audio.play('destroyed');
	suraj.Audio.stop('bg');
	suraj.Audio.stop('wind');
...
suraj.hexgl.ShipControls.prototype.fall = function()
{
	this.active = false;
	this.collision.front = false;
	this.collision.left = false;
...
suraj.hexgl.ShipControls.prototype.update = function(dt)
{
	if(this.falling)
	{
	...
	suraj.Audio.setListenerPos(this.movement);
	suraj.Audio.setListenerVelocity(this.currentVelocity);
};

suraj.hexgl.ShipControls.prototype.teleport = function(pos, quat)
{
...
suraj.hexgl.ShipControls.prototype.boosterCheck = function(dt)
{
...
suraj.hexgl.ShipControls.prototype.collisionCheck = function(dt)
{
...
suraj.hexgl.ShipControls.prototype.heightCheck = function(dt)
{
...
suraj.hexgl.ShipControls.prototype.getRealSpeed = function(scale)
{
	return Math.round(
...
suraj.hexgl.ShipControls.prototype.getRealSpeedRatio = function()
{
	return Math.min(
		this.maxSpeed,
		this.speed+this.boost
...
suraj.hexgl.ShipControls.prototype.getSpeedRatio = function()
{
	return (this.speed+this.boost)/ this.maxSpeed;
};

suraj.hexgl.ShipControls.prototype.getBoostRatio = function()
{
	return this.boost / this.boosterSpeed;
};

suraj.hexgl.ShipControls.prototype.getShieldRatio = function()
{
	return this.shield / this.maxShield;
};

suraj.hexgl.ShipControls.prototype.getShield = function(scale)
{
	return Math.round(
		this.shield
		* (scale == undefined ? 1 : scale)
...
suraj.hexgl.ShipControls.prototype.getPosition = function()
{
	return this.dummy.position;
}

suraj.hexgl.ShipControls.prototype.getQuaternion = function()
{
