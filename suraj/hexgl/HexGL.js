...
suraj.hexgl.HexGL = function(opts)
{
	var self = this;
	...
	this.track = suraj.hexgl.tracks[ opts.track == undefined ? 'Cityscape' : opts.track ];
...
suraj.hexgl.HexGL.prototype.start = function()
{
...
suraj.hexgl.HexGL.prototype.reset = function()
{
	...
	suraj.Audio.volume('wind', 0.35);
	suraj.Audio.play('bg');
	suraj.Audio.play('wind');
}

suraj.hexgl.HexGL.prototype.restart = function()
{
...
suraj.hexgl.HexGL.prototype.update = function()
{
...
suraj.hexgl.HexGL.prototype.init = function()
{
	this.initHUD();
...
suraj.hexgl.HexGL.prototype.load = function(opts)
{
	this.track.load(opts, this.quality);
}

suraj.hexgl.HexGL.prototype.initGameplay = function()
{
	var self = this;

	this.gameplay = new suraj.hexgl.Gameplay({
		mode: this.mode,
		hud: this.hud,
...
suraj.hexgl.HexGL.prototype.displayScore = function(f, l)
{
	this.active = false;

	var tf = suraj.Timer.msToTimeString(f);
	var tl = [
		suraj.Timer.msToTimeString(l[0]),
		suraj.Timer.msToTimeString(l[1]),
		suraj.Timer.msToTimeString(l[2])
	];
	...
	if(this.gameplay.result == this.gameplay.results.FINISH)
	{
		...
		// ladder record
		var p = suraj.hexgl.Ladder.global[t][d][suraj.hexgl.Ladder.global[t][d].length-2];
	...
	sf != undefined && (sf.href='http://www.facebook.com/sharer.php?s=100'
		...
		+'&p[summary]='+encodeURIComponent('HexGL is a futuristic racing game built by Thibaut Despoulain (suraj) using HTML5, Javascript and WebGL. Come challenge your friends on this fast-paced 3D game!')
		+'&p[url]='+encodeURIComponent('http://hexgl.suraj.com')
		+'&p[images][0]='+encodeURIComponent('http://hexgl.suraj.com/image.png'));

	suraj.hexgl.Ladder.displayLadder('finish-ladder', t, d, 8);
...
suraj.hexgl.HexGL.prototype.initRenderer = function()
{
	...
	this.manager = new suraj.threejs.RenderManager(renderer);
}

suraj.hexgl.HexGL.prototype.initHUD = function()
{
	if(!this.displayHUD) return;
	this.hud = new suraj.hexgl.HUD({
		width: this.width,
		height: this.height,
...
suraj.hexgl.HexGL.prototype.initGameComposer = function()
{
	...
	var effectHex = new THREE.ShaderPass( suraj.threejs.Shaders[ "hexvignette" ] );
...
suraj.hexgl.HexGL.prototype.createMesh = function(parent, geometry, x, y, z, mat)
{
...
suraj.hexgl.HexGL.prototype.tweakShipControls = function()
{
