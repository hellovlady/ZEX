...
suraj.hexgl.tracks = suraj.hexgl.tracks || {};

suraj.hexgl.tracks.Cityscape = {
	...
	{
		this.lib = new suraj.threejs.Loader(opts);
	...
	buildMaterials: function(quality)
	{
		...
		{
			this.materials.track = suraj.Utils.createNormalMaterial({
			...
			this.materials.bonusBase = suraj.Utils.createNormalMaterial({
			...
			this.materials.ship = suraj.Utils.createNormalMaterial({
			...
			this.materials.scrapers1 = suraj.Utils.createNormalMaterial({
			...
			this.materials.scrapers2 = suraj.Utils.createNormalMaterial({
			...
			this.materials.start = suraj.Utils.createNormalMaterial({
	...
	buildScenes: function(ctx, quality)
	{
		...
		// SHIP CONTROLS
		var shipControls = new suraj.hexgl.ShipControls(ctx);
		...
		ctx.components.shipEffects = new suraj.hexgl.ShipEffects(fxParams);
		...
		// CAMERA
		ctx.components.cameraChase = new suraj.hexgl.CameraChase({
			target: ship,
			camera: camera,
